...
signal_result_t SignalWaveOutput_Start(signal_wave_output_type_t type,
    float frequency_hz, float vpp_v, float offset_v, float shape_parameter)
{
    ...
    if (!s_initialized ||
        (type < SIGNAL_WAVE_OUTPUT_SINE) ||
        (type > SIGNAL_WAVE_OUTPUT_SAWTOOTH) ||
    ...
    if (((type == SIGNAL_WAVE_OUTPUT_SQUARE) ||
         (type == SIGNAL_WAVE_OUTPUT_SAWTOOTH)) &&
        ((shape_parameter <= 0.0f) ||
         ((type == SIGNAL_WAVE_OUTPUT_SQUARE) &&
          (shape_parameter >= 1.0f)) ||
         ((type == SIGNAL_WAVE_OUTPUT_SAWTOOTH) &&
          (shape_parameter > 1.0f)))) {
...
{ return SignalWaveOutput_Start(SIGNAL_WAVE_OUTPUT_SINE,
    frequency_hz, vpp_v, offset_v, 0.5f); }

signal_result_t SignalWaveOutput_SquareWithOffset(float frequency_hz,
    float vpp_v, float offset_v)
{ return SignalWaveOutput_Start(SIGNAL_WAVE_OUTPUT_SQUARE,
    frequency_hz, vpp_v, offset_v, 0.5f); }
...
signal_result_t SignalWaveOutput_TriangleWithOffset(float frequency_hz,
    float vpp_v, float offset_v)
{ return SignalWaveOutput_Start(SIGNAL_WAVE_OUTPUT_TRIANGLE,
    frequency_hz, vpp_v, offset_v, 0.5f); }

signal_result_t SignalWaveOutput_SawtoothWithOffset(float frequency_hz,
    float vpp_v, float offset_v)
{ return SignalWaveOutput_Start(SIGNAL_WAVE_OUTPUT_SAWTOOTH,
