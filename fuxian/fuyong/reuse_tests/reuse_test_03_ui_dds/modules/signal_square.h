#ifndef SIGNAL_SQUARE_H
#define SIGNAL_SQUARE_H
 ...
 * @param duty_fraction `duty_fraction`（`float`）是该 API 的输入/输出参数；按本 README 前面的数据单位和边界条件准备。
...
/* 兼容旧调用：原接口本来就带 duty_fraction，继续保留避免破坏已有工程。 */
