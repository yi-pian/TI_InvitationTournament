...
比赛拆分顺序：双路同步 ADC 连续采集 -> Ping-Pong 块消费 -> Ring Buffer 事件留存 -> 门限检测猝发长度/高电平点数 -> ST7789 波形页与 4×4 键盘阈值/分页。入口代码在 `main.c`，逐步文档为 `01_模块选择与题目定义.md` 至 `06_验证与赛场复现.md`。

双 ADC 模块负责真实硬件连续块轮转，Ping-Pong 模块只负责应用消费状态，Ring Buffer 保存门限事件；二者没有修改模块源码。SysConfig 仍沿用 22_X 的 ADC、TIMG0、SPI1、ST7789 和固定键盘资源。状态：`BOARD NOT_RUN`。
