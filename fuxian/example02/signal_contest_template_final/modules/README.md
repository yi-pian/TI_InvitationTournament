...
### 4.1 DrawLine 收敛修复（2026-08）

旧版 Bresenham 实现连续两次直接使用并修改 `err`：第一次更新 X 后，第二个判断读到的是修改后的误差，不再是同一个误差快照。对于部分斜线，这会使 `(x0,y0)` 越过目标端点且永远不能满足终止条件，SPI 便持续画点，现象为曲线冲出绘图区后程序卡住。

现版本在每轮先保存 `e2 = 2 * err`，随后两个方向判断都使用相同的 `e2`。这是对核心模块唯一的缺陷修复，不改变 API、SysConfig 或应用层端点映射。已经用所有 `dx=0..319`、`dy=0..239` 的四个方向组合验证，均能在有限步数内到达终点。
