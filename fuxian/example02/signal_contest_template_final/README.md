...
比赛拆分顺序：DDS 波表与 DAC DMA 激励 -> 双路同步 ADC 测量 -> ST7789 幅频/相频曲线 -> 4×4 键盘页面、重扫和采样率控制。入口代码在 `main.c`，逐步文档为 `01_模块选择与题目定义.md` 至 `06_验证与赛场复现.md`。

本题复制 `dds`、`frequency_sweep`、`dac_dma`、`dual_adc`、`phase`、矩阵键盘和 ST7789 README 代码。example02 的 DAC DMA 使用 DMA_CH2，ADC 已占 DMA_CH0/CH1；这是本工程资源冲突后的明确差异，需在 SysConfig 资源冲突视图确认。状态：`BOARD NOT_RUN`。
