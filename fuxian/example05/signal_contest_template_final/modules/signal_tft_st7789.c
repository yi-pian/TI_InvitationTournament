...
{
    int32_t dx=x1>x0?x1-x0:x0-x1, sx=x0<x1?1:-1, dy=y1>y0?y1-y0:y0-y1, sy=y0<y1?1:-1, err=dx-dy;
    tft_st7789_status_t status;
    if (!valid(tft)) return TFT_ST7789_ERROR_ARGUMENT;
    if (!tft->initialized) return TFT_ST7789_ERROR_NOT_INITIALIZED;
    do { if(x0>=0&&y0>=0&&x0<tft->width&&y0<tft->height){status=TFT_ST7789_DrawPixel(tft,x0,y0,color);if(status!=TFT_ST7789_OK)return status;} if(x0==x1&&y0==y1)break; if(2*err>-dy){err-=dy;x0+=sx;} if(2*err<dx){err+=dx;y0+=sy;} } while(1);
    return TFT_ST7789_OK;
}
