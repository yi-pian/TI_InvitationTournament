...
这是 MSPM0G3507 / CCS / SysConfig 的 example06 比赛工程。工程已按比赛步骤选择并复制单 ADC/DMA、ST7789+8×16 字库、4×4 矩阵键盘；CMSIS-DSP FFT 使用 SDK 自带库。详细复现过程见 `example06_总步骤复现说明.md`、五份分步文档及 `example06_05_测试步骤与故障排查.md`。

原双 ADC 文件只保留在 `modules_legacy_dual/` 备份目录，不属于当前 CCS 构建输入；打开工程后请先 Refresh，再 Clean/Build 生成新的 `Debug` 目录。

## 正确使用方法

1. 整个复制/Import 本工程到你的赛题目录。
2. 先判断简单功能是否直接用 SysConfig + TI DriverLib。
3. 确实需要模块时，先打开模块 README，确认模块入口和依赖。
4. 选择本题需要的模块，把 README 明列的 `.c/.h/.inc` 原样复制到 `modules/`。
5. 在 CCS Project Explorer 中 Refresh，确认 `.c` 没有 Exclude from Build。
6. 再按各模块 README 及 `modules/README.md` 的组合补充说明配置本工程
...
| ADC DMA、Timer Capture、DAC DMA、TFT | 按模块 README 配 SysConfig 并复制比赛版文件 |
