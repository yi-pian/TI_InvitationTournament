...
- 当前 CCS SysConfig 1.28.0：生成通过；Event 图为 TIMG0 publisher channel 1 -> ADC0 subscriber channel 1。
- CCS TI Arm Clang 5.1.1.LTS：`-O2 -Wall -Werror` 编译并完整链接通过。
- 链接结果：约 4088 B text、8792 B BSS；BSS 包括 8192 B 的 4096 点缓冲。
- 2026-08-07 LP-MSPM0G3507 实板通过：`g_acceptance_pass=true`、五档 N 全部完成、500/500 帧、WFE 500、哨兵残留 0、FCC PASS；最后一帧 TMP6131 raw min/max/mean 为 2073/2076/2074。
- 当前状态：`BOARD-ONLY VERIFIED`；`PA25 dynamic analog input not verified`，尚未达到 `FULL HARDWARE VERIFIED`。
