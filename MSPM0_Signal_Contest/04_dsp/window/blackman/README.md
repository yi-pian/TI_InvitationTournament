...
> **INTERNAL CHILD / COMPATIBILITY ALIAS：** 正常应用只调用父入口 [`../signal_window.h`](../signal_window.h) 的 `SignalWindow_Apply(..., SIGNAL_WINDOW_BLACKMAN, ...)`。本目录不再作为独立模块选择。

## 1 这个算法是干什么的？

进一步压低频谱旁瓣，帮助在强分量较远处观察弱分量。
...
`w=0.42-0.5cos(2πn/(N-1))+0.08cos(4πn/(N-1))`。更多平滑换来低旁瓣，也换来更宽主瓣。

## 4 比赛里什么时候用？

强基波旁观察弱但相隔较远的杂散/谐波，且能接受更宽峰。
...
`SignalBlackman_Apply(in,out,N,&r);`

## 8 参数怎么改

无参数；近邻分辨差时换 Hann/Rectangular 并比较。

## 9 参数改大会怎样

N 增大可缩小每个 bin 间隔，但观测时间/RAM增大。

## 10 这个算法的代价是什么

Benefits：低旁瓣。Trade-offs：宽主瓣、CG低、计算最多。
...
`Blackman -> FFT -> Magnitude -> GainCorrection`

## 14 最小Demo

```c
signal_window_result_t r;
(void)SignalBlackman_Apply(x,x,N,&r);
```

## 15 PC测试

8 点端点约 0、CG=`0.42*(N-1)/N`，PASS。
...
### `signal_algorithm_status_t SignalBlackman_Apply( const float *input_samples, float *output_samples, uint32_t count, signal_window_result_t *result);`

- **作用：** 应用对称 Blackman 窗，支持原地，并返回实际窗增益。
...
```c
signal_algorithm_status_t status_or_value = SignalBlackman_Apply(input_samples, output_samples, count, result);
```

## 19. Call Sequence / Connecting / Buffer Rules

```text
准备输入/config/workspace -> SignalBlackman_Apply -> 检查返回码 -> 读取 result/output -> 交给下一模块
```
