...
signal_result_t SignalTFTWaveformST7789_MapY(
    float value,
    float scale_minimum,
    float scale_maximum,
    int32_t plot_y,
...
signal_result_t SignalTFTWaveformST7789_GetEnvelopeColumn(
    const float *samples,
    size_t sample_count,
    uint16_t column_count,
    uint16_t column,
    ...
    float scale_maximum)
{
    ...
    if (config->clear_background) {
        status = TFT_ST7789_FillRect(tft, config->x, config->y,
    ...
    if (config->draw_grid) {
        if (config->vertical_grid_divisions > 1U) {
            for (division = 1U;
                ...
                status = TFT_ST7789_DrawLine(tft, coordinate, config->y,
        ...
        if (config->horizontal_grid_divisions > 1U) {
            for (division = 1U;
                ...
                status = TFT_ST7789_DrawLine(tft, config->x, coordinate,
    ...
    if (config->draw_baseline &&
        ...
        status = SignalTFTWaveformST7789_MapY(config->baseline_value,
    ...
    if (config->draw_border) {
        return TFT_ST7789_DrawRect(tft, config->x, config->y,
            (int32_t)config->width, (int32_t)config->height,
            config->grid_color);
    }
    return SIGNAL_RESULT_OK;
    ...
    float scale_maximum)
{
    ...
    for (column = 0U; column < columns; ++column) {
        ...
        status = SignalTFTWaveformST7789_MapY(samples[sample_index],
            scale_minimum, scale_maximum, config->y, config->height, &y);
        if (status != SIGNAL_RESULT_OK) {
            return status;
        }
        if (column == 0U) {
            status = TFT_ST7789_DrawPixel(tft, x, y,
                config->waveform_color);
        } else {
            status = TFT_ST7789_DrawLine(tft, previous_x, previous_y,
    ...
    float scale_maximum)
{
    ...
    for (column = 0U; column < columns; ++column) {
        status = SignalTFTWaveformST7789_GetEnvelopeColumn(samples, sample_count,
            columns, column, &low, &high);
        if (status != SIGNAL_RESULT_OK) {
            return status;
        }
        status = SignalTFTWaveformST7789_MapY(low, scale_minimum, scale_maximum,
            config->y, config->height, &y_low);
        if (status != SIGNAL_RESULT_OK) {
            return status;
        }
        status = SignalTFTWaveformST7789_MapY(high, scale_minimum, scale_maximum,
            config->y, config->height, &y_high);
        if (status != SIGNAL_RESULT_OK) {
            return status;
        }
        x = PlotX(config, column, columns);
        status = TFT_ST7789_DrawLine(tft, x, y_high, x, y_low,
            config->waveform_color);
        if (status != SIGNAL_RESULT_OK) {
            return status;
        }
...
signal_result_t SignalTFTWaveformST7789_Draw(
    tft_st7789_t *tft,
    const float *samples,
    size_t sample_count,
    const signal_tft_waveform_st7789_config_t *config,
