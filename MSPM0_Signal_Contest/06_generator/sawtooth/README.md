...
新接口增加 `symmetry_fraction`，它表示上升段占整个周期的比例，范围为 `0 < x <= 1`。
`1.0f`（100%）是标准上升锯齿，周期末瞬间回落；`0.5f` 表示上升和下降各占半周期；
`0.25f` 表示上升占 25%、下降占 75%。
...
模块不做隐式全局初始化。包含 `signal_sawtooth.h`，由调用者准备配置、缓冲区或平台回调；如头文件提供 Init/Configure，先调用它。

## 7. 调用方法

需要调对称度时调用 `SignalSawtooth_GenerateWithSymmetry`。原 `SignalSawtooth_Generate`
继续保留并固定使用 `1.0f`（100%），已有工程不受影响。

## 8. 参数修改方法

修改 `symmetry_fraction` 即可改变上升/下降时间比例；参数越界返回
`SIGNAL_RESULT_OUT_OF_RANGE`。`rising=false` 会把整个波形上下翻转。
...
~~~c
SignalSawtooth_GenerateWithSymmetry(&table,
    0.5f, 0.4f, 0.0f, true, 1.0f); /* 最后一个参数为 100% 标准上升锯齿 */
~~~
...
当题目需要“sawtooth”目录对应的公开功能，并且输入数据、单位和硬件资源满足前文约束时使用本模块。若只需要更简单的上游功能、输入尚未准备好，或需要不同的数据模型/外设资源，应先选择相邻模块而不是强行调用本 API。
...
```text
SignalSawtooth_Generate -> SignalSawtooth_GetModuleStatus
```
...
| `rising` | `bool` | `rising`（`bool`）是该 API 的输入/输出参数；按本 README 前面的数据单位和边界条件准备。 |

**返回：** 返回 signal_result_t 类型结果；调用者应检查该值。

**最小调用形状：** `SignalSawtooth_Generate(...);`。可直接从 README_MINIMAL_EXAMPLE.c 复制正常流程；README_FULL_EXAMPLE.c 展示全部公开 API，其中取消类 API 会以 #if 0 隔离。

**注意：** 所有指针和数组都由调用者拥有；先检查返回值。异步采集、DMA 或回调还在使用 buffer 时，不能读取结果或改写该 buffer。

### `signal_module_status_t SignalSawtooth_GetModuleStatus();`
...
**最小调用形状：** `SignalSawtooth_GetModuleStatus(...);`。可直接从 README_MINIMAL_EXAMPLE.c 复制正常流程；README_FULL_EXAMPLE.c 展示全部公开 API，其中取消类 API 会以 #if 0 隔离。
