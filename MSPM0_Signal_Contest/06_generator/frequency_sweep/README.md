...
扫频控制逻辑本身是纯软件；接真实激励/采集硬件时，DAC 侧看 [DAC DMA 教程](../../00_docs/SYSCONFIG_BEGINNER_GUIDE.md#dac)，ADC 侧看 [ADC DMA 教程](../../00_docs/SYSCONFIG_BEGINNER_GUIDE.md#adc-timer-dma)，组合基线使用 P04。现场速查见 [Quick Reference](../../00_docs/SYSCONFIG_QUICK_REFERENCE.md)。

无寄存器、引脚或 SysConfig 依赖，可在 PC 上独立测试。

## 6. 初始化方法

模块不做隐式全局初始化。包含 `signal_frequency_sweep.h`，由调用者准备配置、缓冲区或平台回调；如头文件提供 Init/Configure，先调用它。

## 7. 调用方法

`SignalFrequencySweep_Generate`、`SignalFrequencySweep_GetModuleStatus`。
...
当题目需要“frequency_sweep”目录对应的公开功能，并且输入数据、单位和硬件资源满足前文约束时使用本模块。若只需要更简单的上游功能、输入尚未准备好，或需要不同的数据模型/外设资源，应先选择相邻模块而不是强行调用本 API。
...
| 参数 | 类型 | 初学者解释 |
|---|---|---|
| `config` | `const signal_frequency_sweep_config_t *` | 调用者填写的配置对象。先阅读该类型的成员；它控制本次初始化或处理方式。 |
| `frequencies_hz` | `float *` | `frequencies_hz`（`float `）是该 API 的输入/输出参数；按本 README 前面的数据单位和边界条件准备。 |
| `capacity` | `size_t` | 元素数量或容量，单位是“元素个数”而不是字节；必须与实际数组大小一致。 |

**返回：** 返回 signal_result_t 类型结果；调用者应检查该值。

**最小调用形状：** `SignalFrequencySweep_Generate(...);`。可直接从 README_MINIMAL_EXAMPLE.c 复制正常流程；README_FULL_EXAMPLE.c 展示全部公开 API，其中取消类 API 会以 #if 0 隔离。

**注意：** 所有指针和数组都由调用者拥有；先检查返回值。异步采集、DMA 或回调还在使用 buffer 时，不能读取结果或改写该 buffer。

### `signal_module_status_t SignalFrequencySweep_GetModuleStatus();`
...
**最小调用形状：** `SignalFrequencySweep_GetModuleStatus(...);`。可直接从 README_MINIMAL_EXAMPLE.c 复制正常流程；README_FULL_EXAMPLE.c 展示全部公开 API，其中取消类 API 会以 #if 0 隔离。
