...
生成可配置占空比、偏置、幅度和相位的方波表。

## 2. 输入

`offset_fraction`、`amplitude_fraction`、`duty_fraction` 和 `phase_cycles`。
`duty_fraction` 是 0～1 的小数，例如 `0.5f` 表示 50% 占空比，必须严格大于 0 且小于 1。
...
模块不做隐式全局初始化。包含 `signal_square.h`，由调用者准备配置、缓冲区或平台回调；如头文件提供 Init/Configure，先调用它。

## 7. 调用方法

推荐调用 `SignalSquare_GenerateWithDuty`；原 `SignalSquare_Generate` 为兼容接口，参数含义相同。
二者都会直接把可调占空比写入波表。`SignalSquare_GetModuleStatus` 只读取模块状态。

## 8. 参数修改方法

修改 `duty_fraction` 即可改变占空比：`0.2f`=20%，`0.5f`=50%，`0.8f`=80%。
参数不在开区间 `(0, 1)` 时返回 `SIGNAL_RESULT_OUT_OF_RANGE`；不要为某个 Demo 改底层 `.c`。
...
~~~c
SignalSquare_GenerateWithDuty(&table,
    0.5f, 0.4f, 0.5f, 0.0f); /* 偏置、幅度、50% 占空比、相位 */
~~~
...
当题目需要“square”目录对应的公开功能，并且输入数据、单位和硬件资源满足前文约束时使用本模块。若只需要更简单的上游功能、输入尚未准备好，或需要不同的数据模型/外设资源，应先选择相邻模块而不是强行调用本 API。
...
```text
SignalSquare_Generate -> SignalSquare_GetModuleStatus
```
...
| `duty_fraction` | `float` | `duty_fraction`（`float`）是该 API 的输入/输出参数；按本 README 前面的数据单位和边界条件准备。 |
| `phase_cycles` | `float` | `phase_cycles`（`float`）是该 API 的输入/输出参数；按本 README 前面的数据单位和边界条件准备。 |

**返回：** 返回 signal_result_t 类型结果；调用者应检查该值。

**最小调用形状：** `SignalSquare_Generate(...);`。可直接从 README_MINIMAL_EXAMPLE.c 复制正常流程；README_FULL_EXAMPLE.c 展示全部公开 API，其中取消类 API 会以 #if 0 隔离。

**注意：** 所有指针和数组都由调用者拥有；先检查返回值。异步采集、DMA 或回调还在使用 buffer 时，不能读取结果或改写该 buffer。

### `signal_module_status_t SignalSquare_GetModuleStatus();`
...
**最小调用形状：** `SignalSquare_GetModuleStatus(...);`。可直接从 README_MINIMAL_EXAMPLE.c 复制正常流程；README_FULL_EXAMPLE.c 展示全部公开 API，其中取消类 API 会以 #if 0 隔离。
