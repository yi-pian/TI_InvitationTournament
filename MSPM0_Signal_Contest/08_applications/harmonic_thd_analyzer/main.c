...
signal_thd_integration_result_t g_thd_result;
volatile signal_algorithm_status_t g_thd_status;
...
int main(void)
{
    ...
    g_thd_status = SignalIntegration_RawToVoltage(SignalADC_GetBuffer(),
        SignalADC_GetSampleCount(), SIGNAL_ADC_BITS, SIGNAL_ADC_VREF_V,
        SIGNAL_INPUT_SCALE, SIGNAL_INPUT_OFFSET_V, g_voltage,
        SIGNAL_SAMPLE_COUNT);
    if (g_thd_status != SIGNAL_ALGORITHM_OK) goto fail;
    g_thd_status = SignalIntegration_THD(g_voltage, SIGNAL_SAMPLE_COUNT,
        (float) SignalADC_GetConfiguredTriggerRate(),
        (float) SIGNAL_EXPECTED_FREQ_MIN_HZ,
        (float) SIGNAL_EXPECTED_FREQ_MAX_HZ, SIGNAL_HARMONIC_BIN_RADIUS,
        g_fft, SIGNAL_SAMPLE_COUNT, g_magnitude,
        (SIGNAL_SAMPLE_COUNT / 2U) + 1U, &g_thd_result);
    if (g_thd_status != SIGNAL_ALGORITHM_OK) goto fail;
    __BKPT(0);
    while (1) __WFI();
fail:
