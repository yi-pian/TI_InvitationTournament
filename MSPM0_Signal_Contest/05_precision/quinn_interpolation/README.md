# Quinn Second FFT 三点频率估计

这是源码丢失后的 clean reimplementation，明确实现 **Quinn second estimator**；不是旧 `SignalQuinn_Interpolate` 恢复。

## 什么时候想到它

你已有矩形窗复 FFT 和可靠的孤立单音峰，希望用三个复 bin 得到比整数 bin 更细的频率，而且能接受若干除法和 `logf`。

```text
Complex FFT → PeakDetect → Quinn Second → fractional_bin / frequency_hz
```

## 加入和调用

链接 `signal_quinn_interpolation.c`，Include Path 加本目录及 `03_measurement/common`；无需 SysConfig。

```c
signal_quinn_result_t r;
if (SignalQuinnSecond_Process(spectrum, bins, peak, fs, fft_n, &r)
        == SIGNAL_ALGORITHM_OK) {
    use_frequency(r.frequency_hz);
}
```

输入必须是 `signal_complex_f32_t`，不是 magnitude；`peak` 不能为首末元素。适用假设是矩形窗、孤立单音、峰远离 DC/Nyquist。强噪声、多个相邻峰、谐波压住主峰、任意窗函数都会破坏假设。

与 Jacobsen 相比计算更重；与 Macleod 相比含自然对数。比赛现场先用已验证的简单方法，只有扫描证明 Quinn 在当前波形/窗/SNR 下更好时再选。完整公式、来源、失败条件见 [REIMPLEMENTATION_SPEC](REIMPLEMENTATION_SPEC.md)。
...
当题目需要“quinn_interpolation”目录对应的公开功能，并且输入数据、单位和硬件资源满足前文约束时使用本模块。若只需要更简单的上游功能、输入尚未准备好，或需要不同的数据模型/外设资源，应先选择相邻模块而不是强行调用本 API。
...
### `signal_algorithm_status_t SignalQuinnSecond_Process(const signal_complex_f32_t *spectrum, uint32_t spectrum_count, uint32_t peak_index, float sample_rate_hz, uint32_t fft_size, signal_quinn_result_t *result);`
...
| `result` | `signal_quinn_result_t *` | 由调用者分配的输出对象/数组。成功返回后才读取其中内容。 |

**返回：** 返回 signal_algorithm_status_t 类型结果；调用者应检查该值。

**最小调用形状：** `SignalQuinnSecond_Process(...);`。可直接从 README_MINIMAL_EXAMPLE.c 复制正常流程；README_FULL_EXAMPLE.c 展示全部公开 API，其中取消类 API 会以 #if 0 隔离。
