...
### 4.1 已修复：`DrawLine` 的斜率越界/卡死缺陷

2026-08-20 在 LP-MSPM0G3507 实测中发现，旧版 Bresenham 实现连续两次直接读取 `err`：第一次更新 `err` 后，第二个分支错误地以更新后的值判断。这会在部分斜率（例如 `(0,0)->(3,2)`）越过终点，使 `do...while(1)` 永远无法到达目标点，表现为线段越出绘图区并卡在 `TFT_ST7789_DrawLine()`。

修复方式是在每轮开始保存 `e2 = 2 * err`，两个分支都使用同一个 `e2` 判断。这是核心模块缺陷修复，不改变公开 API。已同步到集成库和 example08 的复制模块；旧工程内的历史副本不会自动修改。
