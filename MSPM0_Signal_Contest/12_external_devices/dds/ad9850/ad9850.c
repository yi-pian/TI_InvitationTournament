...
{
    ad9850_status_t status;
    uint8_t phase_b_code;

    if ((device_a == NULL) || (device_b == NULL) ||
        (phase_a_code > 31U) || (phase_difference_code > 31U)) {
        return AD9850_STATUS_BAD_ARGUMENT;
    }

    phase_b_code = (uint8_t) ((phase_a_code + phase_difference_code) & 31U);
    status = AD9850_SetOutput(device_a, frequency_a_hz,
        phase_a_code, power_down_a);
    if (status != AD9850_STATUS_OK) {
        return status;
    }
    return AD9850_SetOutput(device_b, frequency_b_hz,
        phase_b_code, power_down_b);
}
